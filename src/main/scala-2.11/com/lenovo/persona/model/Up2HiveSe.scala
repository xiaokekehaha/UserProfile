...
//      .appName("Fst_Step_Graph_Components")
...
//        }.map(x => (x(0), x(1)))
//      }).filter(x => {
//      if (x._1.trim().last == ':' || x._2.trim().last == ':')
//        false
//      else
//        true
//    }).cache()
...
//    //
//    ConfigUtils.overwriteTextFile("user/hivedata2", data.map(x => x._1 + "," + x._2))
//
//    val dic = data.flatMap(x => List(x._1, x._2)).distinct().zipWithUniqueId()
//    ConfigUtils.overwriteTextFile("user/dic2", dic.map(x => x._1 + "," + x._2))
//
...
//
//
//    //    val data = spark.read.load("user/hivedata").toDF("d1","d2")
//    //    val dic = spark.read.load("user/dic").toDF("e1","e2")
//
//    //    val dic_b = dic.rdd.map( x => (x.getAs[String](0),x.getAs[String](1))).collectAsMap()
//
//
//    //    val data = sc.textFile("/user/hive/warehouse/d_lucp_dw.db/row_data").map(_.split(",")).map(x => (x(0), x(1)))
//    //    val dic = sc.textFile("/user/hive/warehouse/d_lucp_dw.db/dic").map(_.split(",")).map(x => (x(0), x(1).toLong))
//    //    println("开始广播")
//    //    val dic_b = dic.collectAsMap()
//
//    //    val arrBroad = sc.broadcast(dic_b)
//
//
...
//    //    println("rowEdges_1:" + rowEdges_1.count())
...
//
//    //    println("开始join")
//    //    val rowEdges = data.mapPartitions(x => {
//    //      val dicMap = arrBroad.value
//    //      x.map(x => {
//    //        val ed1 = dicMap.get(x._1).get
//    //        val ed2 = dicMap.get(x._2).get
//    //        //        (ed1,ed2)
//    //        ed1 + "," + ed2
//    //      })
//    //    })
//
//    //    val data_df = data.map( x => Row(x._1,x._2))
//
//
//    //    val da = data.join(dic)
//
//    println("开始落地")
//
